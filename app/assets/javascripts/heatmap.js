...
function heatMap(error, data) {
    ...
    width = Math.floor(screen.width * 0.8) - margin.left - margin.right,
    height = Math.floor((screen.height-100) * 0.8) - margin.top - margin.bottom,
    buckets = 10,
    ...
    gridSize = 50,//Math.floor(width / intervals.length),
  ...
  var heatMap = svg.selectAll(".trend")
      .data(data)
      .enter().append("rect")
      .attr("x", function(d) { return (d.interval - 1) * (gridSize / 2); })
